    ...
    const createLauncher = useCallback(
        async (owner: string | undefined, supply: string, tokenAddress: string, data: string) => {
            ...
            try {                
                ...
                console.log("launcher template id: ", tid.toString());
                // get init data
                data = await postauctionlauncher?.getLauncherInitData('', '0x4523ad2e05c455d0043910c84c83236a6c98b40b', owner, owner, 900, 1000);
                await token.approve(liquidityFactoryContract?.address, '1000000000000000000', {from: owner})
                // create
